...
public class DrinkBarServiceImpl implements DrinkBarService {
    ...
    public DrinkBarStateVO getBars(Long playerId) {
        ...
        var drinkViews = recipeConfigService.lambdaQuery()
                ...
                .list()
                .stream()
                .map(recipe -> toDrinkView(playerId, recipe))
                .toList();
        return new DrinkBarStateVO(barViews, drinkViews);
    }
    ...
    public DrinkBarListingResultVO listDrink(Long playerId, Long barId, String recipeId) {
        ...
        if (recipe.getSaleGold() == null || recipe.getSaleExp() == null
                || recipe.getBarSaleIntervalSeconds() == null
                || recipe.getBarSaleIntervalSeconds() <= 0) {
        ...
        int listedQuantity = Math.min(10, inventoryCount);
        ...
        batch.setSaleIntervalSecondsSnapshot(recipe.getBarSaleIntervalSeconds());
    ...
    private DrinkBarStateVO.DrinkView toDrinkView(Long playerId, RecipeConfig recipe) {
        Inventory inventory = inventoryService.findByPlayerAndItem(
                playerId, recipe.getOutputItem());
        int inventoryCount = inventory == null || inventory.getCount() == null
                ? 0 : Math.max(0, inventory.getCount());
        int listingQuantity = Math.min(10, inventoryCount);
        int unitGold = recipe.getSaleGold() == null ? 0 : recipe.getSaleGold();
        int unitExp = recipe.getSaleExp() == null ? 0 : recipe.getSaleExp();
        int interval = recipe.getBarSaleIntervalSeconds() == null
                ? 0 : recipe.getBarSaleIntervalSeconds();

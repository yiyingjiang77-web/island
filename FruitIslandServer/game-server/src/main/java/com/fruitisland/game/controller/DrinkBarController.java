...
public class DrinkBarController {
    ...
    public Result<DrinkBarListingResultVO> listDrink(
        ...
        try {
            ...
            if (!(quantityValue instanceof Number quantity)) return Result.fail("上架数量必须为正整数");
            return Result.ok(drinkBarService.listDrink(
                    player.getId(), barId, recipeId, quantity.intValue()));
        } catch (RuntimeException exception) {
            return Result.fail(exception.getMessage());
        }
    }
